...
let sugesstions = document.querySelector('#sugesstions');

let weatherAPIKey = '5fa58ecdd7c3f65fb76cb46f22e0617f';
let weatherBaseEndpoint = 'https://api.openweathermap.org/data/2.5/weather?units=metric&appid=' + weatherAPIKey;
let forecastBaseEndpoint = 'https://api.openweathermap.org/data/2.5/forecast?units=metric&appid=' + weatherAPIKey;

// Teleport api for city sugesstions
...
let init = () => {
    weatherForCity('London').then(() => document.body.style.filter = 'blure(0)');
}
init();
...
searchInp.addEventListener('input', async () => {
    ...
    for(let i = 0; i < length; i++){
        ...
        sugesstions.appendChild(option);
    }
});
